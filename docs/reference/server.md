...
of which writes anything — and one `stigmergy.repair` module, `schema`, for the removal ledger's
DDL. That slice used to include a governed apply that cloned, gated and pushed from inside an MCP
call, and shrinking it is the whole
...
| `test_service_acl.py` | `BrainService` end to end (fake embedder, real Postgres): ACL enforcement on `search`, `read_page`, `scoped_entities` and `describe_entity`'s own rollup, plus the `max_results` clamp and the structured output shape |
...
| `test_granularity_tripwire_pg.py` | a standing tripwire: the same content filed one-page-per-subject vs bundled into one `entity: []` page, run against both anchoring consumers (`search`'s `entity` filter, `describe_entity`'s timeline) — a bundled page is STRUCTURALLY invisible to either of them, however clearly its body names those subjects |
