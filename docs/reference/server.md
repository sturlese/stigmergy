...
attributed here, and the librarian is what turns it into a page. The exceptions on the whole
server are the review lane's own governed doors, and they are both approvals: approving an
`entity-proposal` mints ([ADR 030](../decisions/030-server-side-entity-minting.md)) and approving a
`repair-proposal` applies the repair a gardener finding earned — or the deletion a person
proposed
...
| `review.py` | `review_queue`/`review_decide` over the **three** kinds a human decides (`entity-proposal`, `parked-capture`, `repair-proposal` — the strings live in `stigmergy.review_kinds`, below both this package and `stigmergy.slack`, whose doorbell renders a card for the first two and skips the third rather than guessing one); the doorbell's read side (`items_for_doorbell`, `load_stewards`, `record_undeliverable`). `reject` and every `parked-capture` verdict never write to git; the two verdicts that do are approvals. Approving an `entity-proposal` mints through the governed door ([ADR 030](../decisions/030-server-side-entity-minting.md)) — `entities.remote.mint_via_clone`, the same `entities.mint.mint` `stigmergy-entities approve`/`create` call, ONE commit, authored as the librarian App with an `Approved-by:` trailer. The three steps of that mint (mint, `review_decisions` row, then the requeue that must never precede the push) are `mint_and_record_approval`, the one function both SERVER-SIDE doors run — this lane and the admin console's Approve. Approving a `repair-proposal` is the second verdict that touches git and has the same shape ([ADR 039](../decisions/039-governed-repair-loop.md)): `apply_repair_and_record` marks the row decided with a conditional UPDATE — which is what makes a second Approve LOSE rather than clone, so no lease is needed anywhere below it — applies through `repair.remote.apply_approved` (the librarian's own `edits.validate`, its eight gates, and a cross-check that the diff is the one the stored `target_paths` describe), and writes the ledger row LAST, after the push. Both doors that approve a repair run that one function — this lane and the console's Repairs Approve — and a gate refusing the apply leaves the proposal `failed` carrying the refusing sentence, never restored to pending: a silent revert would hide that a gate spoke. `stigmergy-entities approve` is a third door that mints: it runs its own copy of the same order from the steward's clone, since `stigmergy.entities` cannot import this package. It records the same ledger row all the same — the writer lives in `capture.decisions`, below both packages — so `review_decisions` answers "who approved this identity" for all three doors, and a CLI mint is attributable from its commit AND from Postgres (issue #51). Every ledger row also names WHICH DOOR recorded it: `review_decide`, `review_decide_safe`, `mint_and_record_approval` and the two repair writers (`apply_repair_and_record`, `reject_repair_and_record`) take a required, undefaulted `source` from `capture.decisions.DECISION_SOURCES`, so a new door that forgets to name itself fails loudly instead of being attributed to an existing one. Authorization is NOT part of `mint_and_record_approval`, nor of `apply_repair_and_record`, and stays per-surface (ADR 030 D2) — which is why the caller set of each is closed and pinned rather than open. A refusal on an item some other door already decided names that decision — verdict, actor, door and time, read out of the ledger — but only ever AFTER the caller has cleared authorization: the `NOT_YOURS_TO_DECIDE` sentence never gains it, or it would become an existence-and-attribution oracle |
...
**The report a `brain_submissions` row carries names unresolved entities in the PLURAL.** A capture
the librarian could not place parks with `unresolved_names` (a list, one entry or many) on the ask
side and `entity_names` on the parked row. The singular `unresolved_name` / `entity_name` are
**legacy, read-only**: nothing writes them any more, and a client reading only the singular finds
nothing on a capture parked after that change. They are not deleted anywhere, because rows written
before it are never migrated. A client that wants one name reads the first entry of the list.
...
`stigmergy.review_kinds.ITEM_KINDS` is the one definition, and it names three:

| Tool | What it does |
|---|---|
| `review_queue(limit?)` | the unified, ACL-scoped inbox over three item kinds: `entity-proposal` (a capture that wants an entity minted), `parked-capture` (a capture waiting on a human answer) and `repair-proposal` (a concrete repair `stigmergy.repair` derived from a gardener finding — an additive edit or a drafted entity body — or a page deletion a person proposed, [repair.md](./repair.md)). An unrestricted (steward) identity sees every item; a scoped identity sees only its own — which is why `repair-proposal` is in the MANAGEMENT read ONLY, and that asymmetry is the honest reading of "its own" rather than an omission: a proposal has no submitter (a nightly job made it, nobody asked for it) and it names page paths, so there is no ownership under which a scoped identity could be shown one. An `entity-proposal` item carries the unresolved identity TWICE, exactly as `entities.situations` emits it: `subject`, one display string joining several names with `", "`, and `subjects`, the per-name list. A consumer that ACTS on a name — running one command per name — reads `subjects`; the joined `subject` is not any of the real names. It also carries `mint_name_prefill`, the name a mint form may default to (`entities.situations.mint_name_prefill`): the single unresolved name, or `""` when several or none are unresolved and no default can be right — `""` too when that one name is the librarian's placeholder for a park that named nothing, which is refused by value so no door offers it ready to mint. A mint surface renders that value and lists `subjects` when it is `""` — it never counts the names itself, so both mint doors agree about when a default is safe |
| `review_decide(item_kind, item_id, verdict, notes?, name?, entity_id?, entity_type?, aliases?, role?, requeue?)` | record a verdict, attributed to the caller's resolved identity, into the append-only `review_decisions` record. Authorization is per kind: an `entity-proposal` decision requires the caller to be a STEWARD (`ops/stewards.json`) and refuses self-approval — the proposer of an entity may never be its own approver; `parked-capture` accepts the row's own submitter or a steward; a `repair-proposal` requires a steward at the scope of EVERY page it would TOUCH. That last one is the first verdict in this lane that can be asked a per-PATH question, and it must be: the other two kinds are anchored to no page, so the universal `"*"` scope is the only one they could resolve, while a repair names the exact pages it would touch and `ops/stewards.json` exists to DELEGATE zones. "Touch" rather than "edit" is load-bearing for the `delete` kind, whose stored paths cover the pages that go AND every page rewritten to stop pointing at them: the steward of a page being removed is not automatically the steward of everybody who mentioned it. It is `all(...)`, not `any(...)` — a contradiction repair edits both sides, so a proposal spanning two zones needs somebody who stewards both, which is no deadlock: either steward may still REJECT it, and the pair can be proposed as two one-sided repairs. `reject` and every `parked-capture` verdict never write to git — Postgres only, categorically. The two paths that do are the two approvals. Approving an `entity-proposal` mints through the governed door ([ADR 030](../decisions/030-server-side-entity-minting.md)): exactly ONE commit, authored as the librarian App with an `Approved-by: <caller>` trailer. That verdict needs `name` (the page title) and `entity_type` (one of `entities.generator.ENTITY_TYPES`) — omitting either is refused, loud and actionable, naming what is missing, and mints nothing; `entity_id` defaults to `name`'s slug, `aliases`/`role` are optional, and `requeue=true` sends the originating capture back to the librarian once the push lands. Approving a `repair-proposal` is the other ([ADR 039](../decisions/039-governed-repair-loop.md)): it applies exactly the approved ops through the librarian's own validator and its eight gates, again ONE App-authored commit with an `Approved-by:` trailer, and the response names the commit and the paths it touched. A gate refusing that apply comes back as the refusing sentence itself — every sentence `repair.remote` raises is written to be read by a steward — and the proposal is already recorded `failed`, never restored to pending. The verdict vocabulary is per kind: `entity-proposal` takes `approve`/`reject` (there is nothing to request changes to — either the name resolves to an identity worth minting or it does not), `repair-proposal` takes the same two for the same shape of reason (a proposal IS its edits, so a different set of edits is a different proposal, and the proposer will make it because a rejected content key is skipped while a re-derived different one is not) with `notes` REQUIRED on the reject, since that reason is the dismissal memory, `parked-capture` takes `capture.dispositions`' own three verbs (`requeue`/`resolve`/`reject`), not a generic `approve`/`reject`/`request_changes` |
...
| `review_queue` / `review_decide` — `CaptureError` (an unknown item kind/verdict, a missing required note, the `NOT_YOURS_TO_DECIDE` authorization refusal, an entity-proposal approve missing `name`/`entity_type`, a mint refusal — a collision, drift, a secret in the role/aliases text, a lost push race, a knowledge repo with no entity template — and, on a repair proposal, a lost race for a row somebody already decided or the sentence a gate raised while applying one) | `{"error": "..."}` | No — the same fixed, no-existence-leak sentence for "does not exist" and "not authorized" alike (`review.NOT_YOURS_TO_DECIDE`); every other message here is built from the caller's own field/verdict value plus static text. The entity-mint refusals mapped in from `entities.errors.EntityError` are the ones to watch, because `entities` writes for a CLI operator and this door republishes to a steward. Two rules hold them: no refusal there may splice the text of an exception `entities` did not itself raise, which `tests/test_architecture.py::test_an_entities_refusal_never_splices_a_caught_exceptions_text` enforces rather than leaving to inspection (it caught three live splices when it was added, one of them an `OSError` carrying an absolute path); and a refusal `entities` DID write for a terminal is re-worded at `entities.remote.mint_via_clone` before it can reach here — the clone-state, template-missing, collision-race and push-race types, which used to arrive naming the server's throwaway clone and a `git -C` command to run in it ([ADR 030](../decisions/030-server-side-entity-minting.md)'s two-door amendment). What still arrives VERBATIM is what a steward can act on: birth-field validation, the collision verdict naming the registered entry, the secrets refusal with its repo-relative page and rule id, and the drift refusal |
