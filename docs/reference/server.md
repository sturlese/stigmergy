...
| `review.py` | `review_queue`/`review_decide` over the **two** kinds a human decides (`entity-proposal`, `parked-capture` — the strings live in `stigmergy.review_kinds`, below both this package and `stigmergy.slack`); the doorbell's read side (`items_for_doorbell`, `load_stewards`, `record_undeliverable`). `reject` and every `parked-capture` verdict never write to git; approving an `entity-proposal` mints through the governed door instead ([ADR 030](../decisions/030-server-side-entity-minting.md)) — `entities.remote.mint_via_clone`, the same `entities.mint.mint` `stigmergy-entities approve`/`create` call, ONE commit, authored as the librarian App with an `Approved-by:` trailer. The three steps of that mint (mint, `review_decisions` row, then the requeue that must never precede the push) are `mint_and_record_approval`, the one function both SERVER-SIDE doors run — this lane and the admin console's Approve. `stigmergy-entities approve` is a third door that mints: it runs its own copy of the same order from the steward's clone, since `stigmergy.entities` cannot import this package. It records the same ledger row all the same — the writer lives in `capture.decisions`, below both packages — so `review_decisions` answers "who approved this identity" for all three doors, and a CLI mint is attributable from its commit AND from Postgres (issue #51). Every ledger row also names WHICH DOOR recorded it: `review_decide`, `review_decide_safe` and `mint_and_record_approval` take a required, undefaulted `source` from `capture.decisions.DECISION_SOURCES`, so a new door that forgets to name itself fails loudly instead of being attributed to an existing one. Authorization is NOT part of `mint_and_record_approval` and stays per-surface (ADR 030 D2). A refusal on an item some other door already decided names that decision — verdict, actor, door and time, read out of the ledger — but only ever AFTER the caller has cleared authorization: the `NOT_YOURS_TO_DECIDE` sentence never gains it, or it would become an existence-and-attribution oracle |
