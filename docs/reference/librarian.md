...
capture_queue row (claimed, fenced by `attempts`)
  ...
  └─ ephemeral git worktree of the knowledge repo
       ...
       │      same for both, only the ENVIRONMENT preamble in front of it differs)
       │
...
library, importable from anywhere). It must **never**
...
whether the page is in this run's own AUDIENCE. Both refusals return the same sentence — "you may
not read that" and "there is no such page"
...
kind and deliberately not a page type.
...
names ([server.md](./server.md#the-capture-tools-the-write-path)). What runs here is the
part that needs a checkout and a credential, and this process is the only one that has either
([`repair.md`](./repair.md) for what the sweep does, and for why it
happens here).
...
the capture is a human act, and
...
knows about it arrives as a capture carrying the four `register_*` hints
