...
capture_queue row (claimed, fenced by `attempts`)
  ...
  └─ ephemeral git worktree of the knowledge repo
       ...
       │    or confirmed when a steward registered it through this capture — appends the
       │    account's facts to entities the registry already knows, and regenerates the
       │    registry: all in the same diff as the page
...
be regenerated by the same function that derives it everywhere else. Every other module here still
imports nothing from that package, and the steward's CLI, the clone discipline and the decision
doors stay out of reach: **the unattended worker writes identities, it never decides them.**
...
| `identity.py` | the identity WRITER: turns the account's `new_entities`/`new_aliases`/`entity_updates` into real files — an entity page per identity (`approved_by: ""`, or the steward's name for the one they registered through this capture), a `proposed_aliases:` line on a registered entity's own page, appended `## Facts` / `## Connections` lines on the pages the account adds to, and the regenerated registry. The ONE module here that may import `stigmergy.entities` |
...
| `STIGMERGY_LIBRARIAN_REPO_URL` (`--url`) | — | inside THIS package, `stigmergy-librarian-boot` only: where a DEPLOYED worker clones `$STIGMERGY_REPO` from, since a container starts with no checkout. The identical env var is also read independently by `stigmergy.server.settings`, for a server-driven entity or repair decision ([ADR 030](../decisions/030-server-side-entity-minting.md)) — see the runbook's [Governing what the librarian proposed](./operator-runbook.md#governing-what-the-librarian-proposed) |
...
writers (`librarian.identity` writing the page, `stigmergy.entities` deciding it), `meeting` the
...
|---|---|---|---|
| identifier (`MAX_IDENTIFIER_LEN`) | `page_path`, `page_type`, `title`, an edit's `path`/`link`, an overlap's `path`, a finding's `category`, a proposed entity's `name`/`entity_type`/`aliases[]`, a proposed alias's `entity`/`alias`, an update's `entity` and each of its `facts[]`/`connections[]` lines | 400 characters | **refused** — it names something the worker resolves, so a longer one is a defect |
...
| proposal count | `new_entities` (`MAX_NEW_ENTITIES`, 10), `new_aliases` (`MAX_NEW_ALIASES`, 20), `entity_updates` (`MAX_ENTITY_UPDATES`, 10) | 10 / 20 / 10 entries | **clipped with a shape finding** — a capture that introduces ten new things is several captures |
...
| `identity/unplanned-entity-edit` | the same for a MODIFIED entity page: an existing identity changes only by a steward's decision, by a repair a steward approved, or by the spelling or the facts `librarian.identity` appended and proved byte for byte |
...
| `identity/not-confirmed-by-its-steward` | the page of an entity a steward REGISTERED through this capture carries some other name in `approved_by`. Only the steward who asked for it may be born standing behind it |
...
the page — and this is the ONLY road into `wiki/entities/`: a proposal, a steward's registration and
a fact added to an entity that already exists all come through here:
...
related, registration, approver)` returns either a `Proposals` record — the registry the commit
will PUBLISH, the created and edited paths, the byte proofs, and the lists the report needs — or a
list of `Finding`s having written **nothing**. It is all-or-nothing like `edits.apply_declared`:
every declaration is judged, so one corrective brief names all of the problems at once.
...
**A page is never born empty.** `entities.birth.render_page` REFUSES a proposal with no What / Who
...
in this brain establish** — search the brain for the name before writing it — so a proposal is not
limited to the one capture that prompted it.
...
### A steward's registration: the one entity born CONFIRMED

A steward introducing an entity nobody has captured about does not write its page either — they
submit what they know as a capture carrying the registration ([ADR
042](../decisions/042-an-entity-is-born-written.md), and
[capture.md](./capture.md#a-registration-is-a-capture-and-only-two-doors-may-assert-one) for the
four hints). This flow is where that capture lands:
...
- **The agent is told.** `agent.build_prompt` appends a server-composed paragraph
  ...
  propose exactly that entity, that the page is the agent's to write from the material AND from the
  existing pages (search the brain for the name first), that the filed page anchors to it — and that
  if the registry ALREADY resolves the name it must anchor to that entity and put the steward's
  spelling in `new_aliases` rather than create a twin.
- **The matching proposal is born confirmed.** `write_proposals(..., registration=, approver=)`
  renders the proposal whose canonical id is the registration's with `approved_by: <steward>` and a
  registry entry that is `proposed: false`. Every OTHER entity the same account proposes stays
  unconfirmed, as always.
- **`registration-missing` is a repairable finding.** An account that neither proposes the entity
  nor resolves the name to a registered one gets `identity/registration-missing`, whose brief
  restates the steward's ask; the one corrective retry acts on it. A name the registry already
  resolves asks nothing — the honest second outcome.
- **The ninth gate checks the name on the page.** `GateContext.confirmed_entity_pages` is
  `{path: approver}`, told by `processing._declare_proposals` like every other widening.
  `gate_identity` requires those pages to say `approved_by: <exactly that steward>`
  (`not-confirmed-by-its-steward`) and every other created entity page to arrive with it empty
  (`approved-on-arrival`). Both are `repairable=False`: code disagreeing with itself.
- **The ledger row comes after the push.** `processing._record_registrations` writes an
  `identity-proposal` APPROVE per confirmed entity — actor the steward, `source` the door
  (`admin` or `cli`), `extra` carrying the commit, `created: true` and the capture id — so the
  decline memory and "entities born" see this door exactly as they see an Approve from the inbox. A
  ledger fault there is logged, never allowed to turn a landed commit into a failed capture.
...
anchored now and a steward confirms, merges or declines the identity from the inbox"*, the spelling
equivalent, and — for a registration — *"It registers 1 new entity: Acme Corp (`acme-corp`),
confirmed by ana@example.com — the page is written from the material and what the brain held, and
the identity is born confirmed, as the steward asked."* The structured `entities_proposed` /
`aliases_proposed` fields carry the same facts for a surface that branches on them, and a registered
entity's entry gains a `confirmed_by` key that an ordinary proposal's three-field shape does not.
Nothing waits on the submitter; the clause exists so that "filed" is not read as "the registry
agreed".
...
`identity.write_proposals` appends each new `facts` line under the page's `## Facts` and each
...
`Proposals.expected_bytes`, so `gate_body_rewrite` proves byte for byte that the commit carries the
bytes the worker planned; `Proposals.updated_pages` / `updates` are what the gates and the report
are told.
...
| `update-of-new-entity` | the named entity is one THIS account also proposes. Its facts belong in that `new_entities` entry, not in a second declaration about it |
...
- `report.py` — every sentence a person reads about a submission. The CLI never composes its own
  ...
  is the ONE sentence saying what a filing did to the identity layer — proposed, registered,
  spelled or added to — and both filed builders (`filed`, `filed_meeting`) append it.
- `identity.write_proposals` — the ONE way an entity page, a proposed spelling or an appended fact
