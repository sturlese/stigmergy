...
next deploy (issue #79). That file
...
| On the page | Means |
|---|---|
| `approved_by: ana@example.com` | the person whose capture introduced this identity. The librarian wrote the page in the commit that filed their capture, and `approved_by` is that capture's `submitted_by`. Nothing waits on anybody: the capture is the approval |
...
platform. That is deliberate: how your agent files pages
