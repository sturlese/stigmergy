...
- **What the sweep does to everything else**, and it is split down the middle (ADR 043 D1). CODE
  ...
  owns the machine zones WHOLE: a `views/` page is regenerated by the view sweep and a `sources/`
  page is a filed document's provenance, so both are unlinked deterministically (`[[X]]` → `X`)
  rather than handed to a writer that would be arguing with a generated file. A MODEL owns the
  bodies of the AUTHORED pages: one call over that set, returning each page's body reconciled, so
