...
| `usage_repair.py` | `ensure_usage_extraction_repaired()` — the shim for `pydantic-ai==2.13.0`'s silent ZERO token extraction (any OpenAI model reporting `output_tokens_details.reasoning_tokens` makes `RequestUsage.extract` raise a `TypeError` inside its own swallowing `try` and fall through to all-zero counts). Idempotent, call-time, and it DEFERS to the original whenever the framework's own extraction worked — so it retires itself on a fixed version. Installed by every agent-construction site in the process (`llm.build_model`, `answer.synthesize.build_synthesizer`, `librarian.pydantic_backend`), because they all share one classmethod |
