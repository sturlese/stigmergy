...
own process. Every terminal and parked status is reported the same way: say so plainly, with the
reason the server gives and nothing added to it.
"""
import asyncio
import logging
...
def _needs_input_prose(report: dict) -> str:
    """Reuse `report.needs_input()`'s situation prose verbatim, swapping ONLY the trailing MCP
    invocation clause — `reply_invocation` is stored beside the sentence for exactly this
    consumer."""
