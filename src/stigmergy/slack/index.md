...
| `store.py` | the package's OWN two durable tables and their DDL — `slack_submissions` (which capture a thread produced, so the poller can reply where it started) and `steward_notifications` (one row per item/steward already DMed, which is what stops the doorbell re-ringing). Nothing rebuilds either; see [Notes](#notes) for why they live here rather than in `stigmergy.capture` |
