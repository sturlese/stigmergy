...
- Bounds: `settings.max_ops_per_proposal` (6) is how much ONE approval may be;
  ...
  never the run). The body road adds `MIN_ANCHORED_PAGES` (2 — below it no model is asked at all,
  whichever of the two checks named the page, and the run records that it was not) and
