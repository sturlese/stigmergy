...
— judged by `gate_body_rewrite`'s permitted-rewrite branch instead of its additive proof. `delete`
is the one kind that REMOVES anything, and the one the covenant reads differently for
...
rather than a judgment. And **a person's own deletion is decided by
...
moves after the push. Its pages are WRITTEN: `deletion` owns the frontmatter, the bounds, and the
machine zones whole — a view is regenerated and a source page is provenance, so both are unlinked
deterministically rather than argued with; `sweep` owns the bodies of the pages a person wrote
(`written_paths`). `entity-alias` is the one
...
spellings that move, the pages that re-anchor, the supersession, the regenerated registry. It is
also the only kind that puts a file which is not a page into a governed commit, and the only one
that reaches `stigmergy.entities`.
...
- `librarian.gather.load_corpus` / `search_candidates` / `confined_page` / `may_read` — one
  ...
  **The proposer's own audience is OPEN, by decision**: a repair has no capture behind it, so no
  human act names an audience for it, and running it
  at open means it may read only open pages and can therefore only ever write open material. The
  price is stated rather than hidden — a restricted page is never repaired by this loop.
...
- Never extend the ADDITIVE op vocabulary past `page.EDIT_KINDS` — a new shape there is a new gate
  ...
  writer, its own branch in the gates and its own place in the gates, which is what `entity-body` and
  `delete` each have.
...
- Never let a MODEL decide WHICH pages a deletion removes. `validate_batch` refuses an op naming a
  ...
  the two are not the same question, and keeping them apart is the whole point of this kind.
...
- Never apply without storing the DIFF. Nobody read the change beforehand, so a row with no diff is
  a change nobody will ever read.
...
- **A `delete` row a person made carries their name in the commit, not in the ledger**: the row is
  ...
  `commit_message` writes when `actor` is set. A worker-derived repair carries
...
- **A merge moves the absorbed entity's ALIASES and never its own name, and that is a constraint
  ...
  apply time. What follows is stated rather than hidden, and is not a defect to be fixed here: the
  absorbed name keeps resolving to the identity the merge retired, whose page now says
  what absorbed it.
...
- Nothing decides a repair. The console's Repairs page READS the ledger — every outcome,
...
[`docs/reference/repair.md`](../../../docs/reference/repair.md).
