...
| `base_inputs.py` | the ONLY way the fast lane reads the ACL config, the entity registry and the contract linter — at `base.sha`, never off the working tree, so a capture is judged by the rules its own base commit carries |
