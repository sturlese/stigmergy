...
(the page on the structured road, the declared edits, and every identity the account proposes).
...
`rejected` or `failed`. A name the registry does not know is PROPOSED — `identity.py` creates the
entity page unconfirmed inside the same commit — and a steward governs it afterwards.
...
`entities.birth`, `entities.generator` and `entities.errors`: a proposed entity's page must be the
same page a steward's `create` renders, and the registry must be regenerated by the same function,
or two renderers would produce two page shapes. The steward's CLI, the clone discipline and the
decision doors stay out of reach — the unattended worker proposes, it never decides.
...
| `identity.py` | the PROPOSER, and the ONE module here that may reach `stigmergy.entities`: `write_proposals` turns an account's `new_entities`/`new_aliases` into a created entity page per identity (`approved_by: ""`, rendered from the knowledge repo's own template through `entities.birth`), a `proposed_aliases:` line on a registered entity's page, and the regenerated registry — or into `Finding`s having written NOTHING. `Proposals` carries what the gates must be told: the registry the commit will PUBLISH, the created and edited paths, the byte proofs, and the two lists the report names |
...
- **An identity is DECLARED by the agent and WRITTEN by code, or it does not exist.** The account
  carries `new_entities`/`new_aliases`; `identity.write_proposals` validates each through the same
  birth gate a steward's `create` passes, writes the files and regenerates the registry;
  `processing._declare_proposals` TELLS the gates what it did (the widened lane, the byte proofs,
  which entity-zone entries are this run's); and `gate_identity` refuses anything else in the zone,
...
- **A gate is TOLD a fact, it never infers one** — the flow-scoped `GateContext` fields default
  ...
  `_declare_proposals`' alone: they are what make "the agent never writes an identity" a proof
  rather than a tool's refusal, and `identity.py` also puts the registry into `derived_files` and
  every touched file into `expected_bytes`, so a "derived" file nobody computed is refused too.
- **Wording for humans lives in `report.py` only**; a read path branches on `reason_code`
  ...
  `failed_system`'s alone. `proposals_clause` is the ONE sentence about a created-unconfirmed
  identity, shared by `filed` and `filed_meeting` so an ordinary capture and a meeting cannot start
...
- **Do not decide a proposal here.** This package proposes; approving, merging and declining are
  `stigmergy.entities`', behind a door that authenticated somebody. The only decision this package
  reads is a DECLINE, from the ledger, so it does not propose the same name forever.
