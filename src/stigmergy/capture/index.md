...
| `decisions.py` | The append-only `review_decisions` ledger — its DDL, its one write (`record_decision`), its one read (`latest_decisions`) and the verdict vocabulary. **Not `dispositions.py`**: that moves a capture between queue STATES and is about the material; this records a governance DECISION about an identity and changes no row's status. It lives here because of who has to WRITE it — all three minting doors, one of which (`stigmergy-entities`) may not import `stigmergy.server`, where it used to live (ADR 030's amendment) |
