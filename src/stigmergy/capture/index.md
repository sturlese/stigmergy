...
| `queue.py` | Insert · claim (`FOR UPDATE SKIP LOCKED`) · `release_expired` · `finish` (the `attempts` fence) · `holds_lease` · the two exits from a park (`record_reply`, `dispose`) · the one listing query and its two entry points · `get_submission_trace` · `query_in_flight` · `work_waiting` (is anything QUEUED right now — one bit, for the worker's view sweep deciding whether to yield the loop back) · `counts_by_status` · the two latency sample readers |
...
| `decisions.py` | The append-only `review_decisions` ledger — its DDL, its one write (`record_decision`), its two reads (`latest_decisions` for every item at once, `latest_decision_for` for one) and the two closed vocabularies: the verdicts, and `DECISION_SOURCES` (which DOOR recorded — `mcp`/`slack`/`admin`/`cli`, required on every write and raised on if unknown, because an append-only row cannot be respelled later). It is stamped into `extra` LAST, so `source` is authoritative: a caller cannot override the validated value through `extra`. **Not `dispositions.py`**: that moves a capture between queue STATES and is about the material; this records a governance DECISION about an identity and changes no row's status. It lives here because of who has to WRITE it — all three minting doors, one of which (`stigmergy-entities`) may not import `stigmergy.server`, where it used to live (ADR 030's amendment) |
