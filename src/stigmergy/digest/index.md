...
corpus health and corpus deltas. It owns no table — it reads `gardener_findings`/`job_runs` and
`capture_queue`/`pages_index`, and writes exactly one thing: its own `job_runs` row, the
watermark. Because it BROADCASTS, every page title it renders passes
...
- Import edges are pinned (`tests/test_architecture.py`): never `stigmergy.views`, `.entities`,
  ...
  `acl.visible` and `errors.StartupError`; no git plumbing, no `wiki/` path literal.
