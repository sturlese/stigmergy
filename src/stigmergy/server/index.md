...
| `review.py` | The two things a person ASKS FOR that this process only queues — it writes nothing to the knowledge repo and holds no credential. `queue_deletion` writes a `delete` capture row carrying the reason and the paths, and the worker performs the removal, commits it with an `Approved-by:` trailer naming the person, and stores the per-page diff on that row. `commission_registration` writes a `raw` row carrying `capture.schema.registration_hints`, and the librarian writes the entity's page and births the identity confirmed by whoever asked. Neither takes an authorization argument: `brain_delete` requires an unrestricted identity at the tool, the console sits behind its operator token, and both caller sets are pinned in `tests/test_architecture.py` |
...
| `identity.py` | The ONE grammar both `ops/` group files are written in — `group_map_from_text` (`{principal: [group, …]}`, whole-file validation, `_`-prefixed keys dropped as comments) and `check_group_names` (the vocabulary's rules, including the reserved `all`), which `slack.channels` parses through too. Above it, `audiences_from_text` (the one parse of `ops/identities.json`, under both the snapshot road and `resolve_audiences`' file road; `UNRESTRICTED_GROUP` membership resolves to `None`) and the per-request half: `hash_token` · `load_token_store` · `resolve_email_for_token`. Fail-closed on every step — an empty text is malformed and resolves nobody |
...
| `acl.py` | `visible(acl, audiences)` — the ONE visibility rule, with its fail-closed truth table. Never re-implement label matching. (`all_visible`, an all-or-nothing companion for text composed from several pages, is gone: it had no caller. The answer layer composes from a service that is already scoped, so there was no per-fragment scoping left for it to make all-or-nothing.) |
...
- `brain_delete` is the one TOOL that touches git, and it decides and writes in the same call —
  because the person calling it is the decision. It authorizes on an
...
- **There is no repair verdict here any more**. The worker derives and applies
