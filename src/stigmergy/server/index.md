...
| `admin` | `review` (`decide_and_record`, `create_and_record`, `apply_repair_and_record`/`reject_repair_and_record`, `VERDICTS_BY_KIND`, `ensure_review_schema` and the review reads), `pilot_report`, `identity.hash_token`, `webhook.JOB_NAME`, `errors` |
...
- Every ledger row names its DOOR. `record_decision` takes a REQUIRED `source` from the closed
  ...
  `create_and_record` thread it undefaulted, so a new door that forgets fails loudly rather
...
- An `entities` exception type never leaves this package through `review_decide` or
  ...
  `decide_and_record`/`create_and_record`, left untranslated on purpose for the single caller
