...
| `webhook.py` | `POST /webhook/github`: HMAC over the RAW body, then a two-phase incremental `pages_index` upsert (`process_push`), split-chain supersession propagation included. Reuses `corpus.page_row` and `store.upsert_pages`/`delete_pages` — never write a second `pages_index` path. It also refreshes the entity-registry snapshot when the push carries `ops/entity-registry.json` (`registry_was_pushed`, asked of the RAW changed paths — `ops/` is in no zone), in the same transaction as the pages and reported as `registry_refreshed` only when it happened — above `store.MAX_ENTITY_REGISTRY_BYTES` it is refused instead (logged, recorded as `registry_refused_bytes`, previous snapshot standing) and the push's pages still land |
...
- Every startup ensures the audit, capture and review schemas behind
  `capture.schema.startup_ddl_lock`, plus `index.store.ensure_entity_registry_table`:
