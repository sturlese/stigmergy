...
def test_ensure_slack_schema_migration_prefers_the_open_capture_when_both_rows_are_real(conn):
    ...
    MOST RECENT mapping, and the loser's id must be named in a WARNING, not silently dropped."""
    notices = []
    ...
    with conn.cursor() as cur:
        ...
        # The capture still in flight.
        cur.execute(
            "INSERT INTO capture_queue (kind, submitted_by, status) VALUES "
            "('raw', 'ana@example.com', 'queued') RETURNING id")
        open_submission_id = cur.fetchone()[0]
        # The FILED capture, mapped first.
        cur.execute(
            "INSERT INTO capture_queue (kind, submitted_by, status) VALUES "
    ...
    with conn.cursor() as cur:
        ...
        rows = cur.fetchall()
    assert len(rows) == 1
    assert rows[0][0] == open_submission_id, "the most recent mapping must survive"
...
def test_dedup_key_migration_prefers_a_real_submission_over_an_orphaned_id(conn):
    """A mapping row whose `submission_id` points at NO `capture_queue` row at all (there is no
    foreign key on this column — see the module docstring) comes back with NULLs through the LEFT
    JOIN. Postgres's default null-ordering is NULLS FIRST under `DESC`, so a plain `DESC` would
    rank the orphan AHEAD of a row with a real capture behind it — the exact opposite of what the
    rule says. `(...) IS TRUE DESC` is what makes the real row survive."""
    with conn.cursor() as cur:
        cur.execute(f"ALTER TABLE slack_submissions DROP CONSTRAINT {store._DEDUP_KEY_NAME}")
        cur.execute(
            "ALTER TABLE slack_submissions ADD CONSTRAINT slack_submissions_legacy_dedup_key "
        ...
        cur.execute(
            "INSERT INTO capture_queue (kind, submitted_by, status) VALUES "
            "('raw', 'ana@example.com', 'queued') RETURNING id")
        real_submission_id = cur.fetchone()[0]
        # An id that names NO capture_queue row at all — orphaned.
        ...
        cur.execute(
            "INSERT INTO slack_submissions (team_id, channel_id, message_ts, thread_ts,"
            " slack_user_id, submitted_by, submission_id) VALUES"
            " ('T5', 'C5', '1.2', '9.5', 'U1', 'ana@example.com', %s)", (real_submission_id,))
    ...
    with conn.cursor() as cur:
        ...
        rows = cur.fetchall()
    assert len(rows) == 1
    assert rows[0][0] == real_submission_id, "the row with a real capture behind it must survive"
...
def test_due_for_report_and_mark_reported_track_status_changes(conn):
    ...
    with conn.cursor() as cur:
        cur.execute("UPDATE capture_queue SET status = 'failed' WHERE id = %s", (submission_id,))
    due = store.due_for_report(conn)
    assert len(due) == 1 and due[0]["status"] == "failed"

    store.mark_reported(conn, reservation_id, "failed")
    assert store.due_for_report(conn) == []   # already reported at this status

    with conn.cursor() as cur:
        # a steward re-ran it by hand and it filed: a FURTHER change reports again
        cur.execute("UPDATE capture_queue SET status = 'filed' WHERE id = %s", (submission_id,))
    due = store.due_for_report(conn)
    assert len(due) == 1 and due[0]["status"] == "filed"
