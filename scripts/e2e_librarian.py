...
environment hygiene; this file assumes the stack is up. Offline and keyless by construction: the
agent is the offline double, and nothing here calls a model.

**Why the bare remote is in a container and not a `tmp_path`.** The pytest suites already push to a
local `git init --bare` and that proves the git mechanics. What it cannot prove is that the commit
leaves the machine's own filesystem conventions behind and lands on a remote reached over a network
protocol by a URL — which is what a real run does, and where a push-authentication or protocol
assumption would break. `git ls-remote` against a `git://` URL is the closest honest stand-in for
GitHub that needs no credential.

**How "serialized commits with zero conflicts" is actually produced.** ONE librarian worker, which
is the supported topology (one worker active by configuration) — and, racing it, a second pusher
standing in for **an operator editing the repo by hand while the librarian pushes to the same
branch**, which is the known race that rebase-and-retry exists for. That is a real contention, on
the real remote, in the supported configuration; running two librarian workers instead would
manufacture contention in a topology nothing runs (and N>1 against one checkout is not currently
safe, because `startup_checks` reaps worktrees the other worker is mid-item in).

This phase is what found the `result_ref` sha defect: a rebase rewrites the commit, so the sha the
report named existed in no reachable history and `git show` on it said `bad object`.
"""
...
# The bare remote the composition publishes. Loopback only, anonymous, and push-enabled — see
# `docker-compose.yml`'s own comment on why that is right here and nowhere else.
REMOTE_URL = os.environ.get("STIGMERGY_E2E_GIT_REMOTE", "git://127.0.0.1:9418/stigmergy.git")

# The knowledge-repo skeleton every checkout is seeded from: the same frozen fixture the librarian
# suites use (a registered entity, `ops/acl.json` in the on-disk dialect, the real contract linter,
# two pre-existing pages). Reused rather than re-invented so the e2e and the suite file against the
# same graph shape, and so a change to one is visible in the other.
...
# Enough ordinary captures that the latency measurement is REAL rather than merely present:
# `latency.MIN_SAMPLES` is the floor below which `status` refuses to print a percentile at all, and
# an e2e that only asserted the field existed would have asserted the refusal.
...
# A figure the material itself supports. This fixture feeds the inverted check below — a body
# full of real numerals is exactly where a resurrected verdict would surface. The double copies the
# material into the body verbatim, which is how the numeral gets onto the page in the first place.
...
# The meeting flow, folded into the SAME drain — `worker.process_next` dispatches by `kind`
# already (no second worker), so this is one more submission and one more set of assertions, not a
# second phase 3. From empty volumes: `stigmergy-meeting drop` -> the worker files -> the page set
# exists on the bare remote. No verification verdict is expected on the source page; nothing
# computes one.
...
def submit_meeting(conn, evidence_store, material: str) -> dict:
    """`stigmergy-meeting drop`'s own enqueue seam (`queue.submit(kind=schema.MEETING, ...)`),
    called directly rather than through a subprocess — the same posture `submit` above already
    takes for the ordinary drop CLI, and consistent with `tests.librarian.support.submit_meeting`,
    which this mirrors."""
...
def assert_parts_carry_no_verdict(source_pages, *, meeting_sha: str, cwd: str, check) -> None:
    ...
    SHARED by both librarian drivers — `e2e_librarian_container.py` imports this module precisely
    so "a change to one is visible in the other", and this assertion is why that matters. It used
    to demand a REAL verification verdict; ingest-time figure verification was removed (ADR 026),
    so it is INVERTED: a verdict here would be a page asserting a check that no longer runs. The
    inversion was applied to the host driver and MISSED on the container twin, which had copied
    the loop instead of calling it — the twin then sat deterministically red until a review found
    it. Hence one function, called twice: the next inversion cannot land on one half.

    `check` is the caller's own recorder (each driver owns its `failures` list), passed in rather
    than imported, so a failure lands in the right driver's summary.
    """
...
def console_command(script: str, module: str) -> list[str]:
    ...
    checkout. Copied in SHAPE from `e2e_write.py`'s own resolver, whose docstring records why
    hardcoding `.venv/bin/...` in one place and not another broke CI at phase 5."""
...
def remote_head() -> str:
    """`main` on the BARE REMOTE, read with `ls-remote` — never from a local clone's stale
    remote-tracking ref. This is the only sha in this file that proves anything left the host."""
...
def submit_all(conn, evidence_store) -> dict:
    """Every capture this run files, submitted in PARALLEL — the "parallel submits" half of the
    property starts here, at the front door, not only at the drain."""
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as pool:
        ...
        # One meeting drop, in the SAME parallel batch — `kind="meeting"` needs its own hints
        # (title, meeting_date), so it goes through `submit_meeting` rather than `submit` above.
...
# A HUMAN's cadence, not a load generator's. Somebody editing a page and pushing it does so every few
# seconds at best; a thread pushing every 0.4s is a fuzzer, and the first cut of this phase was one —
# it lost the librarian three pushes in a row and failed captures nothing conflicted with. What is
# being measured is that rebase-and-retry survives a realistic race, which `human_landed >= 1` plus
# the linear history establishes; hammering the branch measures the retry BUDGET instead, and that
# number belongs in `gitcmd`, where it is now documented.
HUMAN_EDITS = 4
...
def human_editor(checkout: pathlib.Path, stop: list[bool]) -> int:
    ...
    Writes to a `.txt` under `ops/`, deliberately: it is not a page, so it cannot interact with the
    contract linter or the anchoring gate, and the ONLY thing it contributes is a commit on `main`
    that the librarian's next push has to rebase past. Returns how many edits actually landed.
    """
    landed = 0
    for n in range(HUMAN_EDITS):
        ...
        # Rebase onto whatever the librarian pushed since, exactly as a human would: this pusher is
        # not privileged, and a `--force` here would destroy the property the phase is measuring.
        gitcmd.run("fetch", "--quiet", "origin", "main", cwd=str(checkout))
...
def drain(checkout: pathlib.Path, human_checkout: pathlib.Path, conn,
    ...
    Returns `(worker stdout, exit code, human edits landed)`. **stderr is kept SEPARATE and not
    asserted on**, deliberately: the worker logs a per-item fault with `exc_info=True`, so a genuine
    refusal legitimately writes a traceback to the log stream. Merging the two (the first cut did)
    made "no traceback" fail for a worker that was behaving exactly as designed. The property that
    actually means "it did not crash" is the exit code and the closing line, and those are asserted.
    """
...
def main() -> int:
    ...
    try:
        ...
        # From empty volumes the queue IS empty; asserted rather than assumed, because every count
        # below is a count and a leftover row would make all of them wrong in the same direction.
        #
        # **FATAL, not recorded.** `check()` prints and continues, which is right for an assertion
        # about this run's own behaviour and wrong for a precondition about WHICH DATABASE this is:
        # everything below submits 15 fixture captures — one carrying a seeded secret — and drains
        # them, so a non-empty queue means the run is about to mutate a queue it has just discovered
        # is not the disposable one. The DSN is pinned unconditionally in `e2e_librarian.sh`, and
        # this is the second half of the same guarantee: pinned OR refuse, never proceed.
        counts = queue.counts_by_status(conn)
        ...
        # `NEEDS_INPUT` belongs in this set: an unresolved entity is ASKED ABOUT before it is
        # parked for a steward, so the first stop for the unanchorable capture is the submitter's
        # inbox rather than triage.
        ...
        # dedup level 1: ONE page, and the second row closed AT that page as a retry rather than as
        # a rejection — resubmitting identical material is ordinary behavior, not an error
        dup_a, dup_b = rows["duplicate-a"], rows["duplicate-b"]
        # Whichever was processed second is the retry: they were submitted in parallel, so which one
        # that is is genuinely undetermined and the assertion must not pretend otherwise.
        ...
        # INVERTED (ADR 026): ingest-time figure verification is gone, so nothing computes a
        # verdict and nothing may stamp one. The check is kept, pointing the other way — a verdict
        # reappearing over these numerals would be a page ASSERTING a check that no longer runs,
        # which is worse than carrying no verdict at all.
        ...
        # `source_pages` is a LIST — a long transcript is split into cross-linked
        # parts, so the arity is N>=1, not exactly one. This check read the pre-split singular key
        # `source_page`, got `""`, and failed three ways; it is the only thing in the whole suite
        # that noticed the report's contract had changed, because it is the only place that reads
        # the report the way an operator does.
        ...
        # `needs_input`, not `triage`. The capture gets ONE question first — a steward only
        # sees it if the submitter's answer still does not resolve — so parking straight in triage
        # would mean the ask-back never happened.
        ...
        # A FRESH clone, taken after the drain: nothing in it came from the worker's checkout or the
        # human's, so every sha and every byte asserted below is what the remote actually holds.
        ...
        # "meeting" is deliberately EXCLUDED from the generic ordinary-flow accounting below: its
        # commit carries a different subject phrase (`_meeting_commit_message`, "Filed by the
        # librarian's meeting distiller from capture #", not the ordinary flow's "Filed by the
        # librarian from capture #") and adds MULTIPLE files in one commit (source + meeting page +
        # N decisions) — neither matches the ordinary flow's "one page, one commit, one added file"
        # shape this loop asserts. It gets its own dedicated block below instead.
        ...
        # ONE capture, ONE commit — counted against the librarian's own commits rather
        # than against every commit, since the human landed some of the others on purpose. The retry
        # row contributes no commit of its own, which is why this counts DISTINCT `result_ref`s.
        ...
        for ref in sorted(distinct_pages):
            ...
            # `git show <sha>:<path>` run against a clone that only ever saw the remote: this is
            # what proves the sha in a submitter's report is real.
        ...
        # EVERY part is examined, not just the first: each part is a page in its own right, so
        # reading only `source_pages[0]` would leave the split's own pages unexamined — the shape
        # the pre-split check silently had. Shared with the container driver — see the helper.
